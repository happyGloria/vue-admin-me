              ...
              <tr>
                <th v-for='col in colVis'>
                  ...
                  <el-tooltip ref="tooltip"
                    ...
                    <el-button class="btn-help"
                               size="mini">?</el-button>
                  </el-tooltip>
                  <div class="drag"
                       v-drag="col"
                       v-if="column[col[0]].drag !== !1 && !column[col[0]].btnCol"></div>
                </th>
              </tr>
            </thead>
          </table>
        </div>
        <div @mousewheel.stop.prevent="wheelEvent"
             class='jb-c'>
          <table :width="tableWidth"
                 v-if="list.length > 0">
            <colgroup>
            ...
            <tbody class="no-line">
              <tr v-for='(node, index) in blockList'>
                <td v-for='col in colVis'
                    :class="'col-'+column[col[0]].id">
                  <div class="edit"
                       v-if="column[col[0]].btnCol">
                    <slot name="btnCol"
                          :node="node">
                      <el-button type="primary"
                                 size="mini"
                                 class="el-icon-delete"
                                 @click="delNode(node)"></el-button>
                    </slot>
                  ...
                  <div v-else-if="column[col[0]].type === 'select'">
                    <el-form-item errorType="tooltip"
                      ...
                      <el-select v-model="node[column[col[0]].id]">
                        <el-option v-for="item in options(column[col[0]].init(node))"
                                   :key="item.value"
                                   :label="item.label"
                                   :value="item.value">
                        </el-option>
                  ...
                  <div class="ellipsis static-text"
                       v-else>{{node[column[col[0]].id]}}</div>
                </td>
              </tr>
            </tbody>
          </table>
          <table :width="tableWidth"
            ...
            <tbody>
              ...
              <tr>
                <td style="height:70px;line-height:70px;text-align:center;"
                    :colspan="colVis.length">没有添加数据</td>
              </tr>
            </tbody>
          </table>
        </div>
