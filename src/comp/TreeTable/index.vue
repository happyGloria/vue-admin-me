    ...
    <el-table-column v-else
      ...
      <template slot-scope="scope">
        ...
        <span v-if="iconShow(index, scope.row)"
          ...
          <i v-else class="el-icon-minus"/> -->
          <i class="icon" :class="{
            'icon-tree-open': scope.row._expanded,
            'icon-tree-close': !scope.row._expanded
          }" />
        </span>
        <span>
          <i v-if="index == 0" :class="`icon icon-${getIcon(scope.row)}`" />
...
export default {
  ...
  methods: {
    ...
    getIcon({ nodeType, deviceSubType}) {
      return `${nodeType == 'Camera' ? ('Camera-' + deviceSubType) : (nodeType == 'Zone' ? 'Zone' : 'Station')}`
    },
